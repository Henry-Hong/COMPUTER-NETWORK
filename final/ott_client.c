...
#define BUF_SIZE 30 // 사용자 유형에 따라 다르지만 MAX 로 설정
#define NAME_SIZE 20
...
void* recv_msg(void* arg) // read thread main
{
    ...
    char buf[100];
    while (1) {
        packet.command = 0; // file req
        printf("\ninput what u want: ");
        getchar();
        fgets(packet.buf, BUF_SIZE, stdin);
        packet.buf[strlen(packet.buf) - 1] = '\0';
        write(sock, &packet, sizeof(packet)); // 보내고
        read(sock, &packet, sizeof(packet)); // 받기
        if (packet.command == 1) {
